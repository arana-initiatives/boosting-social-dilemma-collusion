...
class CollusionDilemmaEnv(AECEnv):
    ...
    def __init__(self, num_agents=2, hetero_prob=0., eps_len=10, gov_rek=False, zero_mean=False, sparse=False):
        self.sparse = sparse
        ...
        self.payoff_vector = CollusionPayoffs(self._num_agents, self._hetero_prob, self.eps_len, self.gov_rek, self.zero_mean, self.sparse).agent_payoffs
        self.agent_name_mapping = dict(
            zip(self.possible_agents, list(range(len(self.possible_agents))))
        )
        self.agents = self.possible_agents[:]
